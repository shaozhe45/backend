...
public class BaseUncheckedException extends RuntimeException implements BaseException {
    ...
    public BaseUncheckedException(final int code, final String format, Object... args) {
        super(StrUtil.contains(format, BRACE) ? StrUtil.format(format, args) : String.format(format, args));
        this.code = code;
        this.message = StrUtil.contains(format, BRACE) ? StrUtil.format(format, args) : String.format(format, args);
    }
