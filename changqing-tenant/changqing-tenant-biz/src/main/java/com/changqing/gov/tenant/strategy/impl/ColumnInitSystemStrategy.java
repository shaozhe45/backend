...
public class ColumnInitSystemStrategy implements InitSystemStrategy {
    ...
    private boolean initApplication() {
        ...
        list.add(Application.builder().clientId("changqing_ui").clientSecret("changqing_ui_secret").website("http://tangyh.top:10000/changqing-ui/").name("SaaS微服务管理后台").appType(ApplicationAppTypeEnum.PC).status(true).build());
        list.add(Application.builder().clientId("changqing_admin_ui").clientSecret("changqing_admin_ui_secret").website("http://tangyh.top:180/changqing-admin-ui/").name("SaaS微服务管理后台").appType(ApplicationAppTypeEnum.PC).status(true).build());
    ...
    private boolean initMenu(Snowflake snowflake, List<Menu> menuList, Map<String, Long> menuMap) {
        ...
        menuList.add(Menu.builder().id(gatewayId).label("网关管理").describe("网关管理").path("/gateway").component("Layout").icon("el-icon-odometer").sortValue(8).build());
        menuList.add(Menu.builder().id(activitiId).label("流程管理").describe("流程管理").path("/activiti").component("Layout").icon("el-icon-set-up").sortValue(9).build());

        // 2级菜单
        Long orgId = snowflake.nextId();
        menuMap.put(ORG, orgId);
        menuList.add(Menu.builder().id(orgId).parentId(menuUserCenterId).label("组织管理").path("/user/org").component("changqing/user/org/Index").sortValue(0).build());
        Long stationId = snowflake.nextId();
        menuMap.put(STATION, stationId);
        menuList.add(Menu.builder().id(stationId).parentId(menuUserCenterId).label("岗位管理").path("/user/station").component("changqing/user/station/Index").sortValue(1).build());
        Long userId = snowflake.nextId();
        menuMap.put(USER, userId);
        menuList.add(Menu.builder().id(userId).parentId(menuUserCenterId).label("用户管理").path("/user/user").component("changqing/user/user/Index").sortValue(2).build());

        Long roleId = snowflake.nextId();
        menuMap.put(ROLE, roleId);
        menuList.add(Menu.builder().id(roleId).parentId(authId).label("角色管理").path("/auth/role").component("changqing/auth/role/Index").sortValue(1).build());
        Long menuId = snowflake.nextId();
        menuMap.put(MENU, menuId);
        menuList.add(Menu.builder().id(menuId).parentId(authId).label("菜单配置").path("/auth/user").component("changqing/auth/menu/Index").sortValue(2).build());

        Long parameterId = snowflake.nextId();
        menuMap.put(PARAMETER, parameterId);
        menuList.add(Menu.builder().id(parameterId).parentId(baseId).label("参数配置").path("/base/parameter").component("changqing/base/parameter/Index").sortValue(2).build());
        Long dictId = snowflake.nextId();
        menuMap.put(DICT, dictId);
        menuList.add(Menu.builder().id(dictId).parentId(baseId).label("数据字典维护").path("/base/dict").component("changqing/base/dict/Index").sortValue(0).build());
        Long areaId = snowflake.nextId();
        menuMap.put(AREA, areaId);
        menuList.add(Menu.builder().id(areaId).parentId(baseId).label("地区信息维护").path("/base/area").component("changqing/base/area/Index").sortValue(1).build());

        Long applicationApi = snowflake.nextId();
        menuMap.put(APPLICATION, applicationApi);
        menuList.add(Menu.builder().id(applicationApi).parentId(developerId).label("应用管理").path("/developer/application").component("changqing/developer/application/Index").sortValue(1).build());
        Long optLogId = snowflake.nextId();
        menuMap.put(OPT_LOG, optLogId);
        menuList.add(Menu.builder().id(optLogId).parentId(developerId).label("操作日志").path("/developer/optLog").component("changqing/developer/optLog/Index").sortValue(3).build());
        Long loginLogId = snowflake.nextId();
        menuMap.put(LOGIN_LOG, loginLogId);
        menuList.add(Menu.builder().id(loginLogId).parentId(developerId).label("登录日志").path("/developer/loginLog").component("changqing/developer/loginLog/Index").sortValue(4).build());
        Long dbId = snowflake.nextId();
        menuMap.put(DB, dbId);
        menuList.add(Menu.builder().id(dbId).parentId(developerId).label("数据库监控").path("/developer/db").component("changqing/developer/db/Index").sortValue(5).build());
        ...
        menuList.add(Menu.builder().id(serverId).parentId(developerId).label("服务器监控").path("http://127.0.0.1:8762/changqing-monitor").component("Layout").sortValue(9).build());
        Long jobsId = snowflake.nextId();
        menuList.add(Menu.builder().id(jobsId).parentId(developerId).label("定时调度中心").path("http://127.0.0.1:8767/changqing-jobs-server").component("Layout").sortValue(10).build());
        ...
        menuList.add(Menu.builder().id(msgsPushId).parentId(msgsId).label("消息推送").path("/msgs/sendMsgs").component("changqing/msgs/sendMsgs/Index").sortValue(1).build());
        Long myMsgsId = snowflake.nextId();
        menuMap.put(MSGS, myMsgsId);
        menuList.add(Menu.builder().id(myMsgsId).parentId(msgsId).label("我的消息").path("/msgs/myMsgs").component("changqing/msgs/myMsgs/Index").sortValue(2).build());

        Long smsManageId = snowflake.nextId();
        menuMap.put(SMS_MANAGE, smsManageId);
        menuList.add(Menu.builder().id(smsManageId).parentId(smsId).label("短信管理").path("/sms/manage").component("changqing/sms/manage/Index").sortValue(1).build());
        Long templateId = snowflake.nextId();
        menuMap.put(SMS_TEMPLATE, templateId);
        menuList.add(Menu.builder().id(templateId).parentId(smsId).label("账号配置").path("/sms/template").component("changqing/sms/template/Index").sortValue(2).build());

        Long attachmentId = snowflake.nextId();
        menuMap.put(FILE, attachmentId);
        menuList.add(Menu.builder().id(attachmentId).parentId(fileId).label("附件列表").path("/file/attachment").component("changqing/file/attachment/Index").sortValue(1).build());


        menuList.add(Menu.builder().parentId(gatewayId).label("限流规则").path("/gateway/ratelimiter").component("changqing/gateway/ratelimiter/Index").sortValue(1).build());
        menuList.add(Menu.builder().parentId(gatewayId).label("阻止访问").path("/gateway/blocklist").component("changqing/gateway/blocklist/Index").sortValue(2).build());

        Long leaveId = snowflake.nextId();
        Long reId = snowflake.nextId();
        menuList.add(Menu.builder().parentId(activitiId).label("流程部署").path("/activiti/modelManager").component("changqing/activiti/modelManager/Index").sortValue(1).build());
        menuList.add(Menu.builder().parentId(activitiId).label("模型管理").path("/activiti/deploymentManager").component("changqing/activiti/deploymentManager/Index").sortValue(2).build());
        menuList.add(Menu.builder().id(leaveId).parentId(activitiId).label("请假流程").path("/activiti/leave").component("changqing/activiti/leave/Index").sortValue(3).build());
        menuList.add(Menu.builder().id(reId).parentId(activitiId).label("报销流程").path("/activiti/reimbursement").component("changqing/activiti/reimbursement/Index").sortValue(4).build());
        menuList.add(Menu.builder().parentId(leaveId).label("请假管理").path("/activiti/leave/instantManager").component("changqing/activiti/leave/instantManager/Index").sortValue(1).build());
        menuList.add(Menu.builder().parentId(leaveId).label("请假任务").path("/activiti/leave/ruTask").component("changqing/activiti/leave/ruTask/Index").sortValue(2).build());
        menuList.add(Menu.builder().parentId(reId).label("报销管理").path("/activiti/reimbursement/instantManager").component("changqing/activiti/reimbursement/instantManager/Index").sortValue(1).build());
        menuList.add(Menu.builder().parentId(reId).label("报销任务").path("/activiti/reimbursement/ruTask").component("changqing/activiti/reimbursement/ruTask/Index").sortValue(2).build());
