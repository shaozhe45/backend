...
import com.changqing.gov.database.datasource.BaseDatabaseConfiguration;
 ...
 * @date 2021-08-24
 * 断点查看原理：👇👇👇
 * @see com.changqing.gov.database.datasource.BaseMybatisConfiguration#paginationInterceptor()
...
@Slf4j
@MapperScan(
        basePackages = { "com.changqing.gov", "com.changqing.gov"}, annotationClass = Repository.class,
        sqlSessionFactoryRef = GroupDatabaseAutoConfiguration.DATABASE_PREFIX + "SqlSessionFactory")
@EnableConfigurationProperties({MybatisPlusProperties.class})
@ConditionalOnExpression("!'DATASOURCE'.equals('${changqing.database.multiTenantType}')")
public class GroupDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    /**
     * 每个数据源配置不同即可
     */
    final static String DATABASE_PREFIX = "master";

    public GroupDatabaseAutoConfiguration(MybatisPlusProperties properties,
        ...
        log.debug("检测到 zuihou.database.multiTenantType!=DATASOURCE，加载了 GroupDatabaseAutoConfiguration");
    }
